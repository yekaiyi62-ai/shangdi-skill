...
大多数 AI Skill 系统是「女娲模式」——预设了模板，把你的需求填进去。上帝造物术不一样：

| | 女娲模式（模板驱动） | 上帝模式（推演驱动） |
|---|---|---|
| **工作方式** | 选一个模板，填入你的需求 | 理解你的需求，推演出结构 |
| **结果** | 所有备考团队结构相同 | 雅思/CFA/高考 → 完全不同的团队 |
| **弱点发现** | 无 | 能力覆盖检查：发现文档提到「阅读」但没有阅读教练 |
| **记忆系统** | 无或固定结构 | 根据备考时长决定记忆粒度（周/月/季） |
| **角色数量** | 固定 | 从「有几个评分维度」推演出来 |
| **知识读取** | 一次性加载全部 | 按任务按需读取，知识索引驱动 |

上帝在生成团队前会先做一件事：**Phase 0.8 能力覆盖检查**——列出这个领域的必备能力，逐一确认哪个角色覆盖了，如果发现必备能力没人管，立刻停止并告知你，而不是静默跳过生成一个有漏洞的团队。
...
> 生成一个雅思备考团队            → 生成团队（6 教练 + 协调器 + 完整知识库）
...
团队结构：
  ...
  ✓ 语义覆盖：文档中提到的能力（阅读/写作/听力/口语）是否都有对应成员
  ✓ 路由完整性：路由意图数是否能覆盖所有成员
...
- [x] V2.1 质量检查工具（16 项，含语义覆盖 + 路由完整性检查）
- [x] Phase 0.8 能力覆盖检查（生成前强制验证无能力漏洞）
- [x] 雅思阅读教练（V2.1 修复：文档提到阅读但成员缺失）

### 进行中

- [ ] `npx skills add` 一键安装（基础设施准备中）
